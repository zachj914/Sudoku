...
class Grid(Sudoku_Solver.Board):
    def __init__(self, values):
        ...
        self.board_rect = pygame.Rect(screen.width /
    ...
    def draw_board(self):
        ...
        screen.display.blit(self.board, (self.board_rect.left, self.board_rect.top))
    ...
    def draw_square(self, image, cords):
        ...
        screen.display.blit(image, (cords[0], cords[1]))
...
class Screens:
    def __init__(self):
        self.width, self.height = 1080, 773
        self.display = pygame.display.set_mode((self.width, self.height))
        pygame.display.set_caption('Sudoku')
        self.title = pygame.image.load('Titlescreen.jpg').convert()

    def title_display(self):
        self.play_button = Button(pygame.image.load('PlayText.png').convert_alpha(),
                                  horz_alignment=1/2, vert_alignment=7/8)
        self.options_button = Button(pygame.image.load('Options.png').convert_alpha(),
                                     horz_alignment=1/5, vert_alignment=7/8)
        self.leaderboard_button = Button(pygame.image.load('Leaderboard.png').convert_alpha(),
                                         horz_alignment=4/5, vert_alignment=7/8)
        self.active_buttons = {self.play_button:False, self.options_button:False,
                               self.leaderboard_button: False}
        self.display.blit(self.title, (0, 0))
        for button in self.active_buttons.keys():
            if self.active_buttons[button] is True:
                button.draw_active()
            else:
                button.draw()
        pygame.display.flip()
    ...
    def play_display(self):
        self.play = pygame.image.load('Background.jpg').convert()
        self.display.blit(self.play, (0, 0))
        board.draw_board()

    def win_display(self):
        self.win = pygame.image.load('WinScreen.jpg').convert()
...
class Button:
    def __init__(self, image, left_corner=(None, None), horz_alignment=None,
        ...
        if self.left_corner[0] is None:
            self.left_corner[0] = int(screen.width*horz_alignment) - self.image.get_width()/2
        if self.left_corner[1] is None:
            self.left_corner[1] = int(screen.height*vert_alignment) - self.image.get_height()/2
    ...
    def draw_active(self):
        screen.display.blit(self.active_image, self.active_rect)


# Gameloop
def title_loop():
    running = True
    screen.title_display()
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if screen.play_button.rect.collidepoint(mouse):
                    play_loop()
                    running = False

        mouse = pygame.mouse.get_pos()
        prev_buttons = tuple(screen.active_buttons.values())
        for button in screen.active_buttons.keys():
            screen.active_buttons[button] = True if button.rect.collidepoint(mouse) else False
        if tuple(screen.active_buttons.values()) != prev_buttons:
            screen.title_display()
